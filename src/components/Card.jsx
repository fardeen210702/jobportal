...
function Card(el) {
  ...
  return (
    <div className="flex flex-col bg-white shadow-sm shadow-slate-200  transition-all duration-300 ease-in rounded-sm p-2">
      <Link to={`/jobs/${el.job_id}`}>
        <div
          onClick={handleScrollToTop}
          className="w-[350px] md:w-[320px] lg:w-[330px] xl:w-[390px]  text-black 2xl:w-[350px] "
        >
      ...
      <a
        href={job_apply_link} target="_blank"
        className="w-fit bg-white transition-all duration-300 ml-2 py-2  px-5 text-xs  font-semibold text-black rounded-sm  border border-[#E45826] hover:bg-[#E45826] hover:text-white"
      >
        <button>Apply</button>
      </a>
    </div>
